...
router.post(`/signup`, async (req, res) => {
  ...
  try {
    ...
    const resp = sendEmailFun(
      ...
      "Verify Email",
      "",
      "Your OTP is " + verifyCode + "From TheOnlineStore24x7"
    );
...
router.post(`/authWithGoogle`, async (req, res) => {
  ...
  if (!isAllowedAdmin) {
    return res.status(403).send({
      error: true,
      msg: "Access denied: You are not authorized for admin login",
    });
  }
  ...
  try {
    ...
    if (!user) {
      // 🚫 Never take `isAdmin` from frontend
      user = await User.create({
        ...
        password,
        images,
        isAdmin: true, // ✅ Set from backend only
        isVerified: true,
      });
    } else {
      // ✅ Optional: ensure isAdmin is updated in case it wasn't before
      if (!user.isAdmin) {
        user.isAdmin = true;
        await user.save();
      }
...
Your Email Verification OTP is: ${verifyCode}
...
Regards,  
TheOnlineStore24x7 Team
`;
    const html = `
  <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
    <h2 style="color: #2E86C1;">TheOnlineStore24x7</h2>
    <p>Your Email Verification OTP is:</p>
    <h1 style="color: #27AE60;">${verifyCode}</h1>
    <p>This OTP is valid for 10 minutes.</p>
    <p>If you did not request this, please ignore this email.</p>
    <br>
    <p>Regards,<br><strong>TheOnlineStore24x7 Team</strong></p>
  </div>`;
    const resp = sendEmailFun(
      email,
      "Verify Email",
